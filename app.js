...
function renderHeatmap(monthDate, mActivities) {
  ...
  // Build per-day sport data: { counts: {activityId: n}, total }
  const dayData = {};
  mActivities.forEach(a => {
    const day = parseInt(a.date.slice(8));
    if (!dayData[day]) dayData[day] = { counts: {}, total: 0 };
    dayData[day].counts[a.activityId] = (dayData[day].counts[a.activityId] || 0) + 1;
    dayData[day].total++;
  });
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    if (data) {
      const dominant = Object.entries(data.counts).sort((a,b) => b[1]-a[1])[0][0];
      const act = ACT_MAP[dominant] || ACT_MAP['other'];
      const textColor = hexIsLight(act.color) ? '#1C2B2A' : '#fff';
      const multiTip = data.total > 1 ? ` · ${data.total} sessions` : '';
      cells += `<div class="heatmap-cell${todayCls}"
        style="background:${act.color};color:${textColor};"
        title="${act.name}${multiTip}">${day}</div>`;
    } else {
      cells += `<div class="heatmap-cell${todayCls}">${day}</div>`;
    }
  }

  // Legend: only sports that appear this month
  const usedIds = [...new Set(mActivities.map(a => a.activityId))];
  const legendHTML = usedIds.map(id => {
    const act = ACT_MAP[id] || ACT_MAP['other'];
    return `<span style="display:flex;align-items:center;gap:4px;white-space:nowrap">
      <span style="width:12px;height:12px;background:${act.color};border-radius:3px;display:inline-block;flex-shrink:0"></span>
      ${act.icon} ${act.name}
    </span>`;
  }).join('');
  ...
  document.getElementById('calendar-heatmap').innerHTML = `
    ...
    ${legendHTML ? `<div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:12px;font-size:.72rem;color:var(--text-soft);">${legendHTML}</div>` : ''}`;
}
